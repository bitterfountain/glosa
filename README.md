...
(público; los diccionarios conservan las licencias de sus fuentes, ver más abajo).

Lector de libros en el navegador (PDF, EPUB, HTML, TXT) con traducción
instantánea: pulsas cualquier palabra y aparece un popup con su traducción
(diccionario local incrustado) o, si no está, con una consulta online opcional.
Cuatro idiomas (**inglés, español, italiano y alemán**) combinables en las 12
direcciones: cualquiera como idioma de lectura, cualquiera como idioma al que
traducir; la interfaz se muestra en el idioma del lector (el de destino). Además,
**árabe** en las dos direcciones: como destino (español → árabe e inglés → árabe) y como
idioma de lectura (árabe → español y árabe → inglés, con lematizador y lectura de
derecha a izquierda), y **chino** como idioma de lectura (chino → español y chino →
inglés, con segmentación de palabras y conversión a caracteres simplificados); la
interfaz no está en árabe ni en chino.
Sin servidor, sin instalación: también funciona abriendo `index.html`
...
1. Entra en https://glosa.dyndns.org (o abre `index.html` en Chrome, Edge o Firefox:
   todo funciona desde disco salvo el catálogo Top 100, que necesita el proxy del servidor).
2. La primera vez elige tu idioma de lectura y el idioma al que traducir (el segundo viene
   preseleccionado según tu navegador). Se cambian cuando quieras desde las banderas de la barra.
3. Abre un libro (PDF, EPUB, HTML o TXT), arrástralo a la ventana o elige uno del Top 100 de
   Project Gutenberg en inglés, español, italiano o alemán.
4. Pulsa una palabra. Selecciona varias para traducir la frase (consulta online).

Para desarrollar en local basta un servidor estático o `php -S 127.0.0.1:8080` (con PHP,
`index.php` registra la visita y sirve `index.html`; si no existe la carpeta de datos, el
registro se omite en silencio).
...
- **Árabe** (solo como destino, árabe estándar, no dariya): `en-ar.js` viene de FreeDict
...
js/catalog.js         Top 100 de Gutenberg (EN/ES/IT/DE): listas vía proxy /gb/, modal, descarga y apertura
...
dict/<src>-<dst>.js   los 12 diccionarios (generados; ver tabla de arriba)
